package com.kingseducation.app.activities.timetable
...
import com.kingseducation.app.activities.timetable.adapter.TimeTableAllWeekSelectionAdapterNew
import com.kingseducation.app.activities.timetable.adapter.TimeTableSingleWeekSelectionAdapter
import com.kingseducation.app.activities.timetable.adapter.TimeTableWeekListAdapter
import com.kingseducation.app.activities.timetable.model.DayModel
import com.kingseducation.app.activities.timetable.model.FieldModel
import com.kingseducation.app.activities.timetable.model.MondayList
import com.kingseducation.app.activities.timetable.model.PeriodModel
...
//    lateinit var student2timetable: ArrayList<DayModel>
    ...
    private fun callTimeTableApi() {
        ...
        val call: Call<TimeTableResponseModel> = ApiClient.getApiService().timetable(
        ...
        call.enqueue(object : retrofit2.Callback<TimeTableResponseModel> {
            override fun onResponse(
                call: Call<TimeTableResponseModel>, response: Response<TimeTableResponseModel>
            ) {
                    ...
                    if (response.body()!!.status == 100) {
                        ...
                        for (i in 0..feildAPIArrayList.size - 1) {
                            var model = FieldModel(
                                feildAPIArrayList[i].sortname,
                                feildAPIArrayList[i].starttime,
                                feildAPIArrayList[i].endtime
                            )
                        ...
                        if (response.body()!!.timetable.range.Monday.isEmpty() && response.body()!!.timetable.range.Tuesday.isEmpty() && response.body()!!.timetable.range.Wednesday.isEmpty() && response.body()!!.timetable.range.Thursday.isEmpty() && response.body()!!.timetable.range.Friday.isEmpty()) {
                        ...
                        mTimetableApiArrayList.addAll(response.body()!!.timetable.timeTableList)/*for(i in mTimetableApiArrayList.indices)
                        {
                        ...
                        for (f in 0..feildAPIArrayList.size - 1) {
                            ...
                            for (t in 0..mTimetableApiArrayList.size - 1) {
                                if (feildAPIArrayList.get(f).sortname.equals(
                                    ...
                                    Log.e(
                                        "Sortname", mTimetableApiArrayList.get(t).period_name
                                    )
                                    mDayModel.id = mTimetableApiArrayList.get(t).id
                                    mDayModel.period_id = mTimetableApiArrayList.get(t).period_order
                                    mDayModel.day = mTimetableApiArrayList.get(t).day_name
                                    mDayModel.sortname = mTimetableApiArrayList.get(t).period_name
                                    mDayModel.starttime = mTimetableApiArrayList.get(t).start_time
                                    mDayModel.endtime = mTimetableApiArrayList.get(t).end_time
                                    ...
                                    if (mTimetableApiArrayList.get(t).day_name.equals("Monday")) {
                                        ...
                                        dayModel.endtime = mTimetableApiArrayList.get(t).end_time
...
//                    {
                            ...
                            if (weekPosition == 1) {
                                ...
                                var mRecyclerViewMainAdapter = TimeTableSingleWeekSelectionAdapter(
                                    ncontext,
                                    mwednesdayArrayList
                                )
                                timeTableSingleRecycler.adapter = mRecyclerViewMainAdapter
...
//        student2timetable = timeTableListS.shuffle() as ArrayList<DayModel>
        ...
        for (i in weekListArrayString.indices) {
            var weekModel = WeekModel(
                weekListArrayString.get(i).toString(), -1
            )
